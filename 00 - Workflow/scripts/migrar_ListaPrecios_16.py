...
# Archivo de log fijo
log_path = os.path.join(LOG_DIR, "log_lp_R.txt")
...
SHEET_NAME = "Listas anteriores R.old"

# Valores fijos de negocio
ORIGEN = "Odoo8"
ID_LISTA = 16
LISTA = "PUBLICO RAFAELA (PubRAF)"
LOG_DIR = os.path.join(BASE_DIR, "..", "logs")
os.makedirs(LOG_DIR, exist_ok=True)
...
WHEN MATCHED THEN
    UPDATE SET
        ...
        target.precioxkg  = source.precioxkg,
        target.ing_hora   = source.ing_hora
...
for i, row in enumerate(data):
    try:
        ...
        logging.info(
            f"Fila {i} | UPSERT OK | id_lista={row[1]}, cod_art={row[5]}, fecha={row[3]}"
        )
